...
impl<'a> System<'a> for Sys {
    ...
    fn run(
        ...
        /// Trivially cloneable wrapper over any type, to make rayon happy.
        struct OptionClone<T>(Option<T>);

        impl<T> Clone for OptionClone<T> {
            ...
            .for_each_init(
                ...
                |server_emitter, (entity, uid, client, _, pending)| {
                    ...
                    if let Err(e) = || -> Result<(), crate::error::Error> {
                        ...
                        // that we'll definitely be adding a new player.
                        ...
                        // as well as synced resources (currently only
                        // `TimeOfDay`)
