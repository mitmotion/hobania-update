...
float wave_height(vec3 pos) {
    ...
    pos *= 0.5;
    vec3 big_warp = (
        texture(sampler2D(t_waves, s_waves), fract(pos.xy * 0.03 + timer * 0.01)).xyz * 0.5 +
        texture(sampler2D(t_waves, s_waves), fract(pos.yx * 0.03 - timer * 0.01)).xyz * 0.5 +
        vec3(0)
    );
    ...
    float height = (
        ...
        (texture(sampler2D(t_waves, s_waves), (pos.xy + pos.z) * 0.1 + warp.xy + timer * 0.1).x - 0.5) * 0.5 +
        (texture(sampler2D(t_waves, s_waves), (pos.yx + pos.z) * 0.1 + warp.yx - timer * 0.1).x - 0.5) * 0.5 +
