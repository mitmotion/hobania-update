...
pub const DTT: f64 = 1.0 / 100.0;
pub const DT: Duration = Duration::from_millis(10);
...
pub fn get_pos(state: &State, entity: Entity) -> Result<(Pos, Vel, Ori), Box<dyn Error>> {
    let storage = state.ecs().read_storage::<Pos>();
    let pos = storage
        .get(entity)
        .ok_or(Box::new(std::io::Error::new(
            std::io::ErrorKind::Other,
            "Storage does not contain Entity Pos",
        )))?
        .clone();
    ...
    let vel = storage
        ...
        .ok_or(Box::new(std::io::Error::new(
            std::io::ErrorKind::Other,
            "Storage does not contain Entity Vel",
        )))?
        .clone();
    ...
    let ori = storage
        ...
        .ok_or(Box::new(std::io::Error::new(
            std::io::ErrorKind::Other,
            "Storage does not contain Entity Ori",
        )))?
        .clone();
