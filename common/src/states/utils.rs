...
impl Body {
    ...
    /// The swimming force of an entity able to swim (in Newton)
    pub fn can_swim(&self) -> Option<f32> {
        match self {
            Body::Object(_) | Body::Ship(_) => None,
            Body::BipedLarge(_) | Body::Golem(_) => Some(2000.0),
            Body::BipedSmall(_) => Some(500.0),
            Body::BirdMedium(_) => Some(100.0 * self.mass().0),
            Body::BirdSmall(_) => Some(100.0 * self.mass().0),
            Body::FishMedium(_) => Some(10.0 * self.mass().0),
            Body::FishSmall(_) => Some(10.0),
            ...
            Body::Theropod(body) => match body.species {
                ...
                | theropod::Species::Yale => Some(1000.0),
                _ => Some(self.mass().0),
            },
            Body::QuadrupedLow(_) => Some(10.0 * self.mass().0),
            Body::QuadrupedMedium(_) => Some(10.0 * self.mass().0),
            Body::QuadrupedSmall(_) => Some(10.0 * self.mass().0),
        }
    }
    ...
    pub fn can_jump(&self) -> Option<f32> {
        match self {
            Body::Object(_) | Body::Ship(_) => None,
...
pub fn handle_move(data: &JoinData, update: &mut StateUpdate, efficiency: f32) {
    ...
    if input_is_pressed(data, InputKind::Fly)
        ...
        && (!data.physics.on_ground || data.body.can_jump().is_none())
        && data.body.can_fly().is_some()
    {
        fly_move(data, update, efficiency);
    } else if let Some(submersion) = (!data.physics.on_ground && data.body.can_swim().is_some())
...
fn swim_move(data: &JoinData, update: &mut StateUpdate, efficiency: f32, submersion: f32) -> bool {
    if let Some(force) = data.body.can_swim() {
...
pub fn fly_move(data: &JoinData, update: &mut StateUpdate, efficiency: f32) -> bool {
    if let Some(force) = data.body.can_fly() {
